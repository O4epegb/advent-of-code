...
export function part1(instructionList: string): number {
    ...
    while (pointer >= 0 && pointer < instructions.length) {
        const currentInsruction = instructions[pointer];
        instructions[pointer] = currentInsruction + 1;
        pointer = pointer + currentInsruction;
        steps++;
    }
...
export function part2(instructionList: string): number {
    ...
    while (pointer >= 0 && pointer < instructions.length) {
        ...
        instructions[pointer] =
            currentInsruction + (currentInsruction >= 3 ? -1 : 1);
        pointer = pointer + currentInsruction;
        steps++;
    }
